...
brd_type=$(board_type)
if [ "$brd_type" = "Questone2F" ]; then
    echo "Run FSC daemon fand_v2"
    /usr/local/bin/fand_v2
elif [ "$brd_type" = "Seastone2F" ]; then
    echo "Run FSC daemon fand32_v2"
    /usr/local/bin/fand32_v2
elif [ "$brd_type" = "Ivystone" ]; then
    echo "Run FSC daemon fand_ivystone"
    cp /etc/pid_config_v2_ivystone.ini /mnt/data/pid_config_v2.ini -rf
    /usr/local/bin/fand_ivystone
else
